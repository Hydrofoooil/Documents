...
> `apt` 是一个相对较新的、用户友好的前端命令，这意味着 `apt` 命令的输出格式、命令行选项、甚至某些行为在未来的版本更新中不保证保持不变。开发者可能会随时更改它的输出排版或添加、移除参数。

为了确保自动化脚本的稳定性和可靠性，官方强烈建议在脚本中不要使用 `apt`，转而使用更底层、**具有稳定 CLI（命令行接口） 接口**的工具，如：

- **`apt-get`**：用于安装、升级、删除包等操作。
- **`apt-cache`**：用于搜索、查询包信息等操作。
...
- 创建会话
  ...
  ```bash
  tmux new -s my_session  # 把 maoting 换成你喜欢的名字
  ```
...
| **在当前窗口前面插入窗口** | `Ctrl + b` new-window -b [-n 窗口名]           |
| **重命名窗口**       | 按 `Ctrl+b`，然后按 `,`。                         |

- 翻页模式（查看历史输出）

  **进入翻页模式**：按 `Ctrl + b` 然后按 `[` (左中括号)。右上角出现了一个黄色的数字（行号）。

  **开始翻页**：用方向键、PageUp/PageDown 、鼠标滚轮来上下翻看历史输出。

  **退出翻页模式**：按 `q` (Quit)。*一定要按 `q` 退出，否则你没法输入新命令！*

- 开启鼠标模式

  `Ctrl + b`，`:`，状态栏最底部变成了黄色，允许输入命令。
